...
import { SITE, INJURY_TYPES, FILTER_BRANDS, FAQS, calculateIVCSettlement, formatCurrency, IVC_2026 } from "@/lib/calculators/ivc-filter";
...
export default function CalculatorPage() {
    ...
    const [filterBrand, setFilterBrand] = useState("cook");
    const [medicalExpenses, setMedicalExpenses] = useState("50000");
    const [filterStillInBody, setFilterStillInBody] = useState(false);
    const [yearsWithFilter, setYearsWithFilter] = useState("5");
    const [result, setResult] = useState<ReturnType<typeof calculateIVCSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateIVCSettlement(injuryType, filterBrand, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, filterStillInBody, parseInt(yearsWithFilter) || 0));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <Link href="/ivc-filter" className="flex items-center gap-2 hover:opacity-80"><Heart className="w-6 h-6 text-purple-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></Link>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">IVC Filter Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Filter Brand</label><select value={filterBrand} onChange={(e) => setFilterBrand(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{FILTER_BRANDS.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Years with Filter</label><input type="number" value={yearsWithFilter} onChange={(e) => setYearsWithFilter(e.target.value)} min="0" className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                    </div>
                    <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer mt-4"><input type="checkbox" checked={filterStillInBody} onChange={() => setFilterStillInBody(!filterStillInBody)} className="w-4 h-4 text-purple-500" /><span className="text-slate-300 text-sm">Filter is still in body (not removed)</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-red-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-purple-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} • {result.filterBrand}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-purple-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-purple-200">IVC filter settlements have been substantial. Consult an experienced attorney for case evaluation.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="ivc-filter" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{IVC_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
