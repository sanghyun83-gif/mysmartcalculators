...
import { SITE, INJURY_TYPES, ACCIDENT_CAUSES, SCOOTER_COMPANIES, FAQS, calculateEScooterSettlement, formatCurrency, ESCOOTER_2026 } from "@/lib/calculators/e-scooter";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Zap, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("fractures");
    const [accidentCause, setAccidentCause] = useState("vehicle-collision");
    const [scooterCompany, setScooterCompany] = useState("lime");
    const [medicalExpenses, setMedicalExpenses] = useState("45000");
    const [woreHelmet, setWoreHelmet] = useState(false);
    const [companyNegligence, setCompanyNegligence] = useState(true);
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateEScooterSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateEScooterSettlement(injuryType, accidentCause, scooterCompany, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, woreHelmet, companyNegligence, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <Link href="/e-scooter" className="flex items-center gap-2 hover:opacity-80"><Zap className="w-6 h-6 text-amber-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></Link>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">E-Scooter Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Scooter Company</label><select value={scooterCompany} onChange={(e) => setScooterCompany(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SCOOTER_COMPANIES.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={woreHelmet} onChange={() => setWoreHelmet(!woreHelmet)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Wore helmet</span></label>
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={companyNegligence} onChange={() => setCompanyNegligence(!companyNegligence)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Company negligence</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} • {result.scooterCompany}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Not wearing a helmet may reduce recovery in comparative negligence states.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="e-scooter" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{ESCOOTER_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
