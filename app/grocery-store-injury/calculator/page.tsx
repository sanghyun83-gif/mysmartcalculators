...
import { SITE, INJURY_TYPES, INCIDENT_TYPES, FAQS, calculateGrocerySettlement, formatCurrency, GROCERY_2026 } from "@/lib/calculators/grocery-store-injury";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, ShoppingCart, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
...
export default function CalculatorPage() {
    ...
    const [noWarningSign, setNoWarningSign] = useState(true);
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateGrocerySettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateGrocerySettlement(injuryType, incidentType, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, noWarningSign, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Grocery Store Injury Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={noWarningSign} onChange={() => setNoWarningSign(!noWarningSign)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">No warning sign present</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Missing warning signs and inadequate cleanup procedures increase liability.</p></div>
                    </div>
                )}
            </section>
            ...
            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{GROCERY_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
