...
import { SITE, COMPANY_TYPES, REVENUE_TIERS, COVERAGE_LIMITS, FAQS, calculateDOPremium, formatCurrency, DO_2026 } from "@/lib/calculators/do-insurance";
...
export default function CalculatorPage() {
    ...
    const [revenueTier, setRevenueTier] = useState("1m-10m");
    const [coverageLimit, setCoverageLimit] = useState("2m");
    const [boardSize, setBoardSize] = useState("5");
    const [yearsInBusiness, setYearsInBusiness] = useState("5");
    const [hasVCFunding, setHasVCFunding] = useState(false);
    const [claimsHistory, setClaimsHistory] = useState(false);
    const [result, setResult] = useState<ReturnType<typeof calculateDOPremium> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateDOPremium(companyType, revenueTier, coverageLimit, parseInt(boardSize) || 5, parseInt(yearsInBusiness) || 0, hasVCFunding, claimsHistory));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <Link href="/do-insurance" className="flex items-center gap-2 hover:opacity-80"><Users className="w-6 h-6 text-blue-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></Link>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">D&amp;O Insurance Premium Calculator</h1>
                    <p className="text-slate-400">7 input fields for advanced premium estimation.</p>
                </div>

                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Company Type</label><select value={companyType} onChange={(e) => setCompanyType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{COMPANY_TYPES.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Board Size (Directors + Officers)</label><input type="text" value={boardSize} onChange={(e) => setBoardSize(e.target.value.replace(/\D/g, ''))} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={hasVCFunding} onChange={() => setHasVCFunding(!hasVCFunding)} className="w-4 h-4 text-blue-500" /><span className="text-slate-300 text-sm">VC/PE funding</span></label>
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={claimsHistory} onChange={() => setClaimsHistory(!claimsHistory)} className="w-4 h-4 text-blue-500" /><span className="text-slate-300 text-sm">Prior D&amp;O claims</span></label>
                    </div>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-blue-900/30 to-slate-900 border border-blue-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-blue-400">{formatCurrency(result.annualPremium)}/year</p><p className="text-2xl text-emerald-400 mt-2">{formatCurrency(result.monthlyPremium)}/month</p><p className="text-slate-400 mt-2">{result.companyType} • {result.coverageLimit}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg flex items-start gap-3"><TrendingUp className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-blue-200">Ensure adequate Side A coverage. This protects D&amp;Os when the company can&apos;t indemnify.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="do-insurance" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="insurance" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{DO_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
