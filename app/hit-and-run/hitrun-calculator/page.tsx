...
import { SITE, HITRUN_2026, calculateHitRunSettlement, formatCurrency, HitRunResult } from "@/lib/calculators/hit-and-run";

export default function HitRunCalculatorPage() {
    const [sourceIndex, setSourceIndex] = useState(0);
    const [injuryIndex, setInjuryIndex] = useState(1);
    const [age, setAge] = useState("35");
    const [annualIncome, setAnnualIncome] = useState("55,000");
    const [medicalBills, setMedicalBills] = useState("35,000");
    const [hasUM, setHasUM] = useState(true);
    const [result, setResult] = useState<HitRunResult | null>(null);
    ...
    const handleCalculate = () => { const ageNum = parseInt(age) || 35; const incomeNum = parseVal(annualIncome); const medicalNum = parseVal(medicalBills); if (medicalNum > 0) setResult(calculateHitRunSettlement(sourceIndex, injuryIndex, ageNum, incomeNum, medicalNum, hasUM)); };

    return (
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-4">
                    <Link href="/hit-and-run" className="text-slate-400 hover:text-white"><ArrowLeft className="w-6 h-6" /></Link>
                    <div className="flex items-center gap-2"><Car className="w-5 h-5 text-amber-500" /><span className="text-lg font-bold text-white">Hit & Run Calculator</span></div>
            ...
            <main className="max-w-2xl mx-auto px-4 py-8">
                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                    <h1 className="text-xl font-bold text-white mb-2">Hit and Run Settlement Calculator</h1>
                    <p className="text-sm text-slate-400 mb-6">Estimate your hit and run compensation</p>

                    <div className="space-y-5 mb-6">
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Compensation Source</label><select value={sourceIndex} onChange={(e) => setSourceIndex(parseInt(e.target.value))} className="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white">{HITRUN_2026.compensationSources.map((s, i) => (<option key={i} value={i}>{s.source}</option>))}</select><p className="text-xs text-slate-500 mt-1">{HITRUN_2026.compensationSources[sourceIndex].description}</p></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Injury Severity</label><select value={injuryIndex} onChange={(e) => setInjuryIndex(parseInt(e.target.value))} className="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white">{HITRUN_2026.injuryTypes.map((t, i) => (<option key={i} value={i}>{t.type}</option>))}</select></div>
                        ...
                        <div className="flex items-center gap-3"><input type="checkbox" id="hasUM" checked={hasUM} onChange={(e) => setHasUM(e.target.checked)} className="w-5 h-5 rounded border-slate-600 bg-slate-700 text-amber-500" /><label htmlFor="hasUM" className="text-sm text-slate-300">I have Uninsured Motorist (UM) Coverage</label></div>
                    </div>
                ...
                {result && (
                    <div className="mt-6 bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                        ...
                        <div className="bg-amber-900/30 p-4 border-b border-slate-700"><div className="flex items-center justify-center gap-2"><AlertTriangle className="w-4 h-4 text-amber-400" /><span className="text-amber-300">{result.compensationSource} | Max: {formatCurrency(result.maxAvailable)}</span></div></div>
                        <div className="p-6">
                ...
                <section className="bg-slate-800 rounded-xl border border-slate-700 p-6">
                    <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Info className="w-5 h-5 text-amber-500" />Hit & Run FAQ</h2>
                    <div className="space-y-4 text-sm">
                        <div><h3 className="font-semibold text-white mb-1">Can I get compensation if the driver is never found?</h3><p className="text-slate-400">Yes. If you have UM coverage, your own insurance will pay. You may also qualify for state victim compensation funds.</p></div>
                        <div><h3 className="font-semibold text-white mb-1">What is Uninsured Motorist (UM) coverage?</h3><p className="text-slate-400">UM coverage is part of your auto policy that pays when the at-fault driver has no insurance or flees the scene.</p></div>
                    </div>
                </section>
                <p className="mt-8 text-xs text-slate-500 text-center">{HITRUN_2026.citation}</p>
            </main>
            <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [{ "@type": "Question", name: "Can I get compensation if the hit and run driver is never found?", acceptedAnswer: { "@type": "Answer", text: "Yes. If you have UM coverage, your own insurance will pay. You may also qualify for state victim compensation funds." } }] }) }} />
        </div>
    );
}
