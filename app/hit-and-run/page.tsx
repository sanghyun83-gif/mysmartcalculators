    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 py-16 text-center">
          <div className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-full px-4 py-2 mb-6">
            <AlertTriangle className="w-4 h-4 text-red-400" /><span className="text-sm text-red-300">Hit and Run Victim Claims</span>
          </div>
      ...
      <section className="bg-slate-800/50 border-y border-slate-700">
        <div className="max-w-6xl mx-auto px-4 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center"><p className="text-3xl font-bold text-amber-400">{(HITRUN_2026.statistics.annualHitRuns / 1000).toFixed(0)}K</p><p className="text-sm text-slate-400 mt-1">Hit & Runs/Year</p></div>
            <div className="text-center"><p className="text-3xl font-bold text-red-400">{HITRUN_2026.statistics.annualDeaths.toLocaleString()}</p><p className="text-sm text-slate-400 mt-1">Deaths/Year</p></div>
            <div className="text-center"><p className="text-3xl font-bold text-yellow-400">{HITRUN_2026.statistics.unsolvedPercent}%</p><p className="text-sm text-slate-400 mt-1">Unsolved</p></div>
            <div className="text-center"><p className="text-3xl font-bold text-orange-400">{HITRUN_2026.statistics.nighttimePercent}%</p><p className="text-sm text-slate-400 mt-1">At Night</p></div>
          </div>
        </div>
      </section>

      <section className="max-w-6xl mx-auto px-4 py-16">
        <h2 className="text-2xl font-bold text-white mb-8 text-center">Free Hit & Run Tools</h2>
      ...
      <section className="bg-slate-800/30 border-y border-slate-700">
        <div className="max-w-4xl mx-auto px-4 py-16">
          <h2 className="text-2xl font-bold text-white mb-8 text-center">Compensation Sources</h2>
          <div className="space-y-4">{HITRUN_2026.compensationSources.map((s, i) => (<div key={i} className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex justify-between items-center"><div><h3 className="text-white font-medium">{s.source}</h3><p className="text-sm text-slate-400">{s.description}</p></div><div className="text-right"><p className="text-lg font-bold text-amber-400">{formatCurrency(s.maxPayout)}</p><p className="text-xs text-slate-500">Max Payout</p></div></div>))}</div>
        </div>
      </section>
      ...
      <footer className="bg-slate-800 border-t border-slate-700">
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-sm text-slate-400 text-center">{HITRUN_2026.citation}. For informational purposes only.</p>
