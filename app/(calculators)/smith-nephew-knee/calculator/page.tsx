...
import { SITE, INJURY_TYPES, SN_KNEE_PRODUCTS, FAQS, calculateSNKneeSettlement, formatCurrency, SN_KNEE_2026 } from "@/lib/calculators/smith-nephew-knee";
...
export default function CalculatorPage() {
    ...
    const [productType, setProductType] = useState("journey");
    const [medicalExpenses, setMedicalExpenses] = useState("70000");
    const [yearsSinceImplant, setYearsSinceImplant] = useState("4");
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateSNKneeSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateSNKneeSettlement(injuryType, productType, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(yearsSinceImplant) || 0, hasDocumentation));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">S&amp;N Knee Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Product Type</label><select value={productType} onChange={(e) => setProductType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SN_KNEE_PRODUCTS.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                    ...
                    <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer mt-4"><input type="checkbox" checked={hasDocumentation} onChange={() => setHasDocumentation(!hasDocumentation)} className="w-4 h-4 text-purple-500" /><span className="text-slate-300 text-sm">Have implant records and revision surgery documentation</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-red-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Early failure cases (under 5 years) receive higher compensation multipliers.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="smith-nephew-knee" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{SN_KNEE_2026.citations.join(" ??")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the smith nephew knee calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this smith nephew knee calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the smith nephew knee data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these smith nephew knee results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
