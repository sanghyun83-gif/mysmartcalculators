...
import { SITE, INJURY_TYPES, MESH_MANUFACTURERS, SURGERY_COMPLICATIONS, FAQS, calculateMeshSettlement, formatCurrency, MESH_2026 } from "@/lib/calculators/transvaginal-mesh";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Heart, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("erosion-revision");
    const [manufacturer, setManufacturer] = useState("jnj");
    const [surgeryComplication, setSurgeryComplication] = useState("multiple-revision");
    const [medicalExpenses, setMedicalExpenses] = useState("100000");
    const [yearsSinceImplant, setYearsSinceImplant] = useState("8");
    const [hasRevisionSurgery, setHasRevisionSurgery] = useState(true);
    const [hasOngoingSymptoms, setHasOngoingSymptoms] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateMeshSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateMeshSettlement(injuryType, manufacturer, surgeryComplication, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(yearsSinceImplant) || 0, hasRevisionSurgery, hasOngoingSymptoms));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Mesh Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Mesh Manufacturer</label><select value={manufacturer} onChange={(e) => setManufacturer(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{MESH_MANUFACTURERS.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Surgery Status</label><select value={surgeryComplication} onChange={(e) => setSurgeryComplication(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SURGERY_COMPLICATIONS.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={hasRevisionSurgery} onChange={() => setHasRevisionSurgery(!hasRevisionSurgery)} className="w-4 h-4 text-purple-500" /><span className="text-slate-300 text-sm">Had revision surgery</span></label>
                    ...
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-rose-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="bg-slate-800/50 rounded-xl p-4 mt-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Surgery Status</span><span className="text-white">{result.surgeryComplication}</span></div>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="transvaginal-mesh" count={5} /></div></div>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the transvaginal mesh calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this transvaginal mesh calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the transvaginal mesh data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these transvaginal mesh results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
