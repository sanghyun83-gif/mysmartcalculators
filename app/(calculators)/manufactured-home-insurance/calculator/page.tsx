...
import { Factory, Calculator, ArrowRight, DollarSign } from "lucide-react";
import { SITE, HOME_VALUES, FAQS } from "@/lib/calculators/manufactured-home-insurance";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function ManufacturedHomeInsuranceCalculatorPage() {
    const [homeValue, setHomeValue] = useState("");
    const [dwellingCoverage, setDwellingCoverage] = useState(100000);
    ...
    const calculatePremium = () => {
        ...
        const dwellingRate = (dwellingCoverage / 1000) * 5.5 * tier.factor;
        const propertyRate = 175;
        const liabilityRate = 90;
        const anchorDiscount = isAnchored ? 0.85 : 1.0;

        const annual = Math.round((dwellingRate + propertyRate + liabilityRate) * anchorDiscount);
            ...
            <section className="py-12 px-4">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-10">
                        ...
                        <h1 className="text-3xl font-bold text-white mb-2">Manufactured Home Insurance Calculator</h1>
                        <p className="text-slate-400">Estimate your HUD-code home insurance costs</p>
                    </div>
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 mb-8">
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="range" min="30000" max="300000" step="10000" value={dwellingCoverage} onChange={(e) => setDwellingCoverage(Number(e.target.value))} className="w-full" />
                            </div>

                            <div>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={isAnchored} onChange={(e) => setIsAnchored(e.target.checked)} className="rounded bg-slate-700 border-slate-600" />
                                    <span className="text-slate-300">Permanently Anchored (15% discount)</span>
                                </label>
                            </div>
                        </div>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the manufactured home insurance calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this manufactured home insurance calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the manufactured home insurance data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these manufactured home insurance results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
