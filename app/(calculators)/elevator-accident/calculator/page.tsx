...
import { SITE, INJURY_TYPES, ACCIDENT_CAUSES, FAQS, calculateElevatorSettlement, formatCurrency, ELEVATOR_2026 } from "@/lib/calculators/elevator-accident";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Building2, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("fractures");
    const [accidentCause, setAccidentCause] = useState("door-injury");
    ...
    const [result, setResult] = useState<ReturnType<typeof calculateElevatorSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateElevatorSettlement(injuryType, accidentCause, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, maintenanceNegligence, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Elevator Accident Settlement Calculator</h1>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Inspect maintenance records - poor maintenance history significantly increases settlement value.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="elevator-accident" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{ELEVATOR_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
