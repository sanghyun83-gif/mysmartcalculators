...
export default function CalculatorPage() {
    ...
    const [bloodPFASLevel, setBloodPFASLevel] = useState("high");
    const [result, setResult] = useState<ReturnType<typeof calculatePFASSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculatePFASSettlement(exposureType, cancerType, parseInt(yearsExposed) || 5, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(lostWages.replace(/\D/g, '')) || 0, parseInt(futureCareCosts.replace(/\D/g, '')) || 0, bloodPFASLevel));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">PFAS Lawsuit Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Cancer/Disease Type</label><select value={cancerType} onChange={(e) => setCancerType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{CANCER_TYPES.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Blood PFAS Level</label><select value={bloodPFASLevel} onChange={(e) => setBloodPFASLevel(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white"><option value="high">High (Tested)</option><option value="medium">Medium</option><option value="unknown">Unknown</option></select></div>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-orange-900/20 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.exposureType}  {result.cancerType}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">PFAS cases are actively being litigated. Consult with an attorney experienced in MDL 2873 for accurate case evaluation.</p></div>
                    </div>
                )}
            </section>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the pfas calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this pfas calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the pfas data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these pfas results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
