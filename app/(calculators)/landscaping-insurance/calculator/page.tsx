...
import { Trees, Calculator, ArrowRight, DollarSign } from "lucide-react";
import { SITE, SERVICE_TYPES, REVENUE_TIERS, FAQS } from "@/lib/calculators/landscaping-insurance";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function LandscapingInsuranceCalculatorPage() {
    ...
    const calculatePremium = () => {
        ...
        if (!service || !revenueTier) return;

        let glPremium = 1800 * service.factor * revenueTier.factor;
        let wcPremium = includeWC ? 3500 * (employees / 2) * revenueTier.factor : 0;
        let equipmentPremium = includeEquipment ? 600 : 0;
        let autoPremium = 1800;

        const annual = Math.round(glPremium + wcPremium + equipmentPremium + autoPremium);
        const monthly = Math.round(annual / 12);

        const breakdown = [
            { label: "General Liability", value: service.name, cost: `$${Math.round(glPremium).toLocaleString()}` },
            ...(includeWC ? [{ label: "Workers Compensation", value: `${employees} employees`, cost: `$${Math.round(wcPremium).toLocaleString()}` }] : []),
            ...(includeEquipment ? [{ label: "Equipment Coverage", value: "Included", cost: `$${equipmentPremium.toLocaleString()}` }] : []),
            { label: "Commercial Auto", value: "Trucks/Trailers", cost: `$${autoPremium.toLocaleString()}` },
        ];
            ...
            <section className="py-12 px-4">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-10">
                        ...
                        <h1 className="text-3xl font-bold text-white mb-2">Landscaping Insurance Calculator</h1>
                        <p className="text-slate-400">Estimate your landscaping contractor insurance costs</p>
                    </div>
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 mb-8">
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select value={serviceType} onChange={(e) => setServiceType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <label className="flex items-center gap-2">
                                        <input type="checkbox" checked={includeEquipment} onChange={(e) => setIncludeEquipment(e.target.checked)} className="rounded bg-slate-700 border-slate-600" />
                                        <span className="text-slate-300">Equipment Coverage ($600/yr)</span>
                                    </label>
                                </div>
                            </div>
                        </div>

                        <button onClick={calculatePremium} disabled={!serviceType || !revenue} className="w-full mt-6 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white py-4 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors">
                    ...
                    {result && (
                        <div className="bg-blue-900/30 border border-blue-500/50 rounded-xl p-6 mb-8">
                            ...
                            <p className="text-xs text-slate-500 mt-4">*Estimates based on 2026 NCCI landscaping classification rates</p>
                        </div>
                    )}
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the landscaping insurance calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this landscaping insurance calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the landscaping insurance data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these landscaping insurance results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
