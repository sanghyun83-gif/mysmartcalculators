...
import { SITE, INJURY_TYPES, CLAIM_TYPES, ACCIDENT_CAUSES, FAQS, calculateTrainSettlement, formatCurrency, TRAIN_2026 } from "@/lib/calculators/train-accident";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Train, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("fractures");
    const [claimType, setClaimType] = useState("fela");
    const [accidentCause, setAccidentCause] = useState("negligence");
    const [medicalExpenses, setMedicalExpenses] = useState("95000");
    const [isRailroadWorker, setIsRailroadWorker] = useState(true);
    const [railroadNegligence, setRailroadNegligence] = useState(true);
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateTrainSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateTrainSettlement(injuryType, claimType, accidentCause, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, isRailroadWorker, railroadNegligence, hasDocumentation));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Train Accident Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={isRailroadWorker} onChange={() => setIsRailroadWorker(!isRailroadWorker)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Railroad worker (FELA)</span></label>
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={railroadNegligence} onChange={() => setRailroadNegligence(!railroadNegligence)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Railroad negligence</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} ??{result.claimType}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">FELA claims and proven negligence typically result in higher settlements.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="train-accident" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{TRAIN_2026.citations.join(" ??")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the train accident calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this train accident calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the train accident data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these train accident results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
