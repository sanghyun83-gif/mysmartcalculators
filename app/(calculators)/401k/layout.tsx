...
import { ChevronRight, TrendingUp, Star, ArrowLeft } from "lucide-react";
import { GlobalFooter } from "@/components/GlobalFooter";

export default function TrendingUpFlagshipLayout({
    children,
}: {
    ...
    const isHub = pathname === "/401k";

    // S-Class Theme DNA: Dynamic Color Injection
    const themeStyles = {
        '--accent-rgb': '59, 130, 246',      // Red-500
    ...
    return (
        <div
            className="min-h-screen bg-[#0a0f1a] text-white selection:bg-blue-500/30"
            style={themeStyles}
        >
                ...
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-3">
                            <Link href="/401k" className="p-2 bg-blue-500 rounded-lg shadow-[0_0_20px_rgba(239,68,68,0.3)] hover:scale-105 transition-transform">
                                <TrendingUp className="w-5 h-5 text-white" />
                            </Link>
                            <div className="flex flex-col">
                                <Link href="/401k" className="text-xl font-black tracking-tighter uppercase leading-none hover:text-blue-500 transition-colors">
                                    FinancePro AI
                                </Link>
                        ...
                        <div className="hidden lg:flex items-center gap-6 text-[10px] font-black text-slate-500 uppercase tracking-widest border-l border-white/10 pl-6 h-8">
                            <Link href="/401k#stats" className="hover:text-white transition-colors">BENCHMARKS</Link>
                            <Link href="/401k#rules" className="hover:text-white transition-colors">LEGAL FRAMEWORK</Link>
                            <Link href="/401k#trinity" className="hover:text-white transition-colors">CASE INTELLIGENCE</Link>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        {!isHub && (
                            <Link href="/401k" className="flex items-center gap-2 text-[10px] font-black text-white px-4 py-2 bg-white/5 rounded-xl hover:bg-white/10 transition-all uppercase tracking-widest border border-white/10">
            ...
            <div className="flex flex-col min-h-screen">
                    ...
                    <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-black tracking-tighter uppercase">FinancePro AI</span>
                            </div>
                        ...
                        <div className="md:text-right">
                            <div className="inline-flex items-center gap-3 px-6 py-3 bg-white/5 rounded-2xl border border-white/10 group">
                                <div className="flex -space-x-3">
                                    {[1, 2, 3].map(i => (
                                        <div key={i} className="w-8 h-8 rounded-full border-2 border-black bg-slate-800 flex items-center justify-center text-[8px] font-black uppercase text-blue-500">DA</div>
                                    ))}
                                </div>
                ...
                <GlobalFooter cite="FinancePro AI Intelligence Unit" />
            </div>
        </div>
    );
}
