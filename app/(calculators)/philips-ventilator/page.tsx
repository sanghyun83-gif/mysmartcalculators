...
import { SITE, CALCULATORS, INJURY_TYPES, PHILIPS_2026, formatCurrency } from "@/lib/calculators/philips-ventilator";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { ArrowRight, AlertTriangle, Wind, Scale } from "lucide-react";

export const metadata = { title: `${SITE.name} | Free ${SITE.year} CPAP Recall Calculator`, description: SITE.description };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-2"><Wind className="w-6 h-6 text-purple-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></div>
            ...
            <section className="relative overflow-hidden">
                ...
                <div className="relative max-w-4xl mx-auto px-4 py-16 text-center">
                    ...
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Philips CPAP<span className="block text-purple-400">Lawsuit Calculator</span></h1>
                    <p className="text-lg text-slate-300 mb-8 max-w-2xl mx-auto">Calculate Philips CPAP/BiPAP lawsuit settlements. Foam degradation, carcinogenic particles, respiratory damage.</p>
                    <Link href="/philips-ventilator/calculator" className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-105">Calculate Settlement<ArrowRight className="w-5 h-5" /></Link>
                </div>
            </section>
            ...
            <section className="bg-slate-800/50 border-y border-slate-700">
                <div className="max-w-6xl mx-auto px-4 py-12">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                        <div><p className="text-3xl font-bold text-purple-400">{PHILIPS_2026.statistics.mdlNumber}</p><p className="text-sm text-slate-400 mt-1">MDL Number</p></div>
                        <div><p className="text-3xl font-bold text-red-400">{PHILIPS_2026.statistics.devicesRecalled / 1000000}M+</p><p className="text-sm text-slate-400 mt-1">Devices Recalled</p></div>
                        <div><p className="text-3xl font-bold text-amber-400">Class I</p><p className="text-sm text-slate-400 mt-1">FDA Recall</p></div>
            ...
            <section className="max-w-6xl mx-auto px-4 py-16">
                <h2 className="text-2xl font-bold text-white mb-8 text-center">Philips CPAP Tools</h2>
            ...
            <section className="max-w-4xl mx-auto px-4 py-16 text-center">
                ...
                <Link href="/philips-ventilator/calculator" className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-8 py-4 rounded-xl font-semibold">Start Free Calculator<ArrowRight className="w-5 h-5" /></Link>
            </section>
            ...
            <footer className="bg-slate-800 border-t border-slate-700">
                <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-2"><Wind className="w-5 h-5 text-purple-500" /><span className="font-semibold text-white">{SITE.name}</span></div>
                    <p className="text-sm text-slate-400">{PHILIPS_2026.citations[0]}</p>
