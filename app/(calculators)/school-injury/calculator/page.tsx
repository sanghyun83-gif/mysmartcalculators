...
import { SITE, INJURY_TYPES, INCIDENT_TYPES, SCHOOL_TYPES, FAQS, calculateSchoolSettlement, formatCurrency, SCHOOL_2026 } from "@/lib/calculators/school-injury";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, School, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("fractures");
    const [incidentType, setIncidentType] = useState("playground");
    const [schoolType, setSchoolType] = useState("public");
    const [medicalExpenses, setMedicalExpenses] = useState("30000");
    const [priorComplaints, setPriorComplaints] = useState(false);
    const [bullyingInvolved, setBullyingInvolved] = useState(false);
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateSchoolSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateSchoolSettlement(injuryType, incidentType, schoolType, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, priorComplaints, bullyingInvolved, hasDocumentation));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">School Injury Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">School Type</label><select value={schoolType} onChange={(e) => setSchoolType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SCHOOL_TYPES.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={priorComplaints} onChange={() => setPriorComplaints(!priorComplaints)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Prior complaints</span></label>
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={bullyingInvolved} onChange={() => setBullyingInvolved(!bullyingInvolved)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Bullying involved</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} ??{result.incidentType}</p></div>
                        <div className="bg-slate-800/50 rounded-xl p-4 mt-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between py-2"><span className="text-slate-400">School Type</span><span className="text-white">{result.schoolType}</span></div>
                            </div>
                        </div>
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Public school claims require tort claims notice within 60-180 days. Consult an attorney immediately.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="school-injury" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{SCHOOL_2026.citations.join(" ??")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the school injury calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this school injury calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the school injury data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these school injury results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
