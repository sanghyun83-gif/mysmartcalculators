...
import { SITE, INJURY_TYPES, DEPUY_KNEE_PRODUCTS, FAILURE_TIMING, FAQS, calculateDePuyKneeSettlement, formatCurrency, DEPUY_KNEE_2026 } from "@/lib/calculators/depuy-knee";
...
export default function CalculatorPage() {
    ...
    const [productType, setProductType] = useState("attune");
    const [failureTiming, setFailureTiming] = useState("within-2");
    ...
    const [result, setResult] = useState<ReturnType<typeof calculateDePuyKneeSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateDePuyKneeSettlement(injuryType, productType, failureTiming, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, hadRevisionSurgery, hasOngoingSymptoms, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">DePuy Knee Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Product Type</label><select value={productType} onChange={(e) => setProductType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{DEPUY_KNEE_PRODUCTS.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Time to Failure</label><select value={failureTiming} onChange={(e) => setFailureTiming(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{FAILURE_TIMING.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
                    ...
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-red-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="bg-slate-800/50 rounded-xl p-4 mt-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between py-2 border-b border-slate-700"><span className="text-slate-400">Time to Failure</span><span className="text-white">{result.failureTiming}</span></div>
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Early failure (within 2 years) and Attune knee system cases receive highest compensation.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="depuy-knee" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{DEPUY_KNEE_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
