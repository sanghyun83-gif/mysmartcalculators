...
const FAQS = [
  ...
  {
    question: "Are the state agency source rows official?",
    answer:
...
function FAQSection() {
  return (
    <div className="max-w-4xl mx-auto px-4 space-y-2">
      {FAQS.map((faq, index) => (
        <details key={`${faq.question}-${index}`} className="group bg-white border border-slate-200 rounded-md hover:border-slate-300">
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <header className="pt-6 pb-2 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-1 text-[11px] text-slate-500 font-mono mb-4 uppercase tracking-wider"><ShieldCheck size={14} className="text-blue-600" />Verified by state agencies + lifecycle assumptions + source log</div>
      </header>
      ...
      <article className="py-10 max-w-5xl mx-auto px-6 space-y-8">
        ...
        <section className="bg-white border border-slate-200 shadow-sm rounded-md p-4"><h2 className="text-base font-bold text-slate-900 mb-2">State Coverage Links</h2><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">{capRanking.slice(0, 9).map((state) => (<Link key={state.code} className="rounded-md border border-slate-200 bg-white px-3 py-2 hover:bg-slate-50" href={`/workers-comp/${getWorkersCompStateSlug(state.code)}`}>{state.name} workers comp page</Link>))}</div></section>
