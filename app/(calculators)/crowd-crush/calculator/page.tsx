...
import { SITE, INJURY_TYPES, EVENT_TYPES, DENSITY_LEVELS, FAQS, calculateCrushSettlement, formatCurrency, CRUSH_2026 } from "@/lib/calculators/crowd-crush";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Users, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("asphyxiation");
    const [eventType, setEventType] = useState("concert");
    const [densityLevel, setDensityLevel] = useState("severe");
    const [medicalExpenses, setMedicalExpenses] = useState("50000");
    const [warningsIgnored, setWarningsIgnored] = useState(true);
    const [capacityExceeded, setCapacityExceeded] = useState(false);
    const [hasDocumentation, setHasDocumentation] = useState(true);
    const [result, setResult] = useState<ReturnType<typeof calculateCrushSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateCrushSettlement(injuryType, eventType, densityLevel, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, warningsIgnored, capacityExceeded, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <Link href="/crowd-crush" className="flex items-center gap-2 hover:opacity-80"><Users className="w-6 h-6 text-amber-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></Link>
                    <span className="text-xs text-amber-400 bg-amber-500/20 px-2 py-1 rounded border border-amber-500/30">Advanced</span>
                </div>
            </header>

            <div className="max-w-6xl mx-auto px-4 py-3"><Link href="/crowd-crush" className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-amber-400"><ArrowLeft className="w-4 h-4" />Back</Link></div>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Crowd Crush Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Event Type</label><select value={eventType} onChange={(e) => setEventType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{EVENT_TYPES.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Crowd Density</label><select value={densityLevel} onChange={(e) => setDensityLevel(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{DENSITY_LEVELS.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 mt-4">
                        <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer"><input type="checkbox" checked={warningsIgnored} onChange={() => setWarningsIgnored(!warningsIgnored)} className="w-4 h-4 text-amber-500" /><span className="text-slate-300 text-sm">Warnings ignored</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-slate-900 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} • {result.eventType} • {result.densityLevel}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-red-200">Ignored warnings and exceeded capacity significantly increase liability and settlement values.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="crowd-crush" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{CRUSH_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
