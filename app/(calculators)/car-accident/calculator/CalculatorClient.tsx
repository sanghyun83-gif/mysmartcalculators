                        ...
                        <div className="p-12 bg-gradient-to-br from-[#0c111d] to-black border border-red-500/20 rounded-[4rem] shadow-2xl space-y-12 relative overflow-hidden group">
                            ...
                            <div className="grid gap-4 pt-10 border-t border-white/5">
                                <ResultMetric
                                    ...
                                    description="Direct financial losses including medical bills and lost wages."
                                />
                                <ResultMetric
                                    label="Pain & Suffering"
                                    value={formatCurrency(results.breakdown.painSuffering)}
                                    description="Non-economic damages based on injury severity staging."
                                    variant="highlight"
                                />
                                <ResultMetric
                                    label="Fault Reduction"
                                    value={`-${formatCurrency(results.breakdown.faultReduction)}`}
                                    description={`Reduction based on ${faultPercentage}% fault allocation.`}
                                    variant="critical"
                                    trend={{ type: 'down', label: `${faultPercentage}% Fault` }}
                                />
                            </div>
