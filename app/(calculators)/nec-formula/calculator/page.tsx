...
import { SITE, NEC_SEVERITY, FORMULA_USED, FAQS, calculateNECSettlement, formatCurrency, NEC_2026 } from "@/lib/calculators/nec-formula";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Baby, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [severity, setSeverity] = useState("surgery");
    const [formulaUsed, setFormulaUsed] = useState("similac");
    const [medicalExpenses, setMedicalExpenses] = useState("150000");
    const [hadSurgery, setHadSurgery] = useState(true);
    const [birthWeeks, setBirthWeeks] = useState("30");
    const [result, setResult] = useState<ReturnType<typeof calculateNECSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateNECSettlement(severity, formulaUsed, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, hadSurgery, parseInt(birthWeeks) || 37));
    };
    ...
    return (
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">NEC Severity</label><select value={severity} onChange={(e) => setSeverity(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{NEC_SEVERITY.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Formula Used in NICU</label><select value={formulaUsed} onChange={(e) => setFormulaUsed(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{FORMULA_USED.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Birth Weeks (Gestational Age)</label><input type="number" value={birthWeeks} onChange={(e) => setBirthWeeks(e.target.value)} min="22" max="42" className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                    </div>
                    <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer mt-4"><input type="checkbox" checked={hadSurgery} onChange={() => setHadSurgery(!hadSurgery)} className="w-4 h-4 text-purple-500" /><span className="text-slate-300 text-sm">Required bowel surgery (resection)</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-rose-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-purple-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-purple-200">NEC litigation is ongoing. Bellwether trials in 2026 will influence outcomes. Consult an attorney.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="nec-formula" count={5} /></div></div>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the nec formula calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this nec formula calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the nec formula data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these nec formula results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
