...
import { SITE, SILICOSIS_TYPES, SEVERITY_LEVELS, FAQS, calculateSilicosisSettlement, formatCurrency, SILICOSIS_2026 } from "@/lib/calculators/silicosis";
...
export default function CalculatorPage() {
    const [silicosisType, setSilicosisType] = useState("chronic");
    const [severity, setSeverity] = useState("moderate");
    const [medicalExpenses, setMedicalExpenses] = useState("60000");
    const [lostWages, setLostWages] = useState("100000");
    const [yearsExposed, setYearsExposed] = useState("12");
    const [result, setResult] = useState<ReturnType<typeof calculateSilicosisSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateSilicosisSettlement(silicosisType, severity, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(lostWages.replace(/\D/g, '')) || 0, parseInt(yearsExposed) || 10));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Silicosis Settlement Calculator</h1>
                    <p className="text-slate-400">Estimate your silica exposure compensation based on disease type and severity.</p>
                </div>

                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Silicosis Type</label><select value={silicosisType} onChange={(e) => setSilicosisType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SILICOSIS_TYPES.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Severity</label><select value={severity} onChange={(e) => setSeverity(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{SEVERITY_LEVELS.map((s) => <option key={s.id} value={s.id}>{s.name} - {s.description}</option>)}</select></div>
                        ...
                        <div className="md:col-span-2"><label className="block text-sm font-medium text-slate-300 mb-2">Years of Exposure</label><input type="number" value={yearsExposed} onChange={(e) => setYearsExposed(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                    </div>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-indigo-900/30 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Estimate only. Actual settlements vary. Consult an attorney for accurate evaluation.</p></div>
                    </div>
                )}
            </section>
                ...
                <div className="space-y-4">
                    {FAQS.map((faq, i) => (
                        <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                            <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left"><span className="text-white font-medium pr-4">{faq.question}</span>{openFaq === i ? <ChevronUp className="w-5 h-5 text-purple-400 flex-shrink-0" /> : <ChevronDown className="w-5 h-5 text-slate-400 flex-shrink-0" />}</button>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="silicosis" count={5} /></div></div>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the silicosis calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this silicosis calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the silicosis data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these silicosis results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
