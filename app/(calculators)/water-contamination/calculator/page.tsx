...
import { SITE, CONTAMINATION_TYPES, HEALTH_EFFECTS, EXPOSURE_DURATION, FAQS, calculateWaterContaminationSettlement, formatCurrency, WATER_CONTAMINATION_2026 } from "@/lib/calculators/water-contamination";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Droplets, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [contaminationType, setContaminationType] = useState("pfas");
    const [healthEffect, setHealthEffect] = useState("cancer");
    const [exposureDuration, setExposureDuration] = useState("long");
    const [medicalExpenses, setMedicalExpenses] = useState("100000");
    const [lostWages, setLostWages] = useState("75000");
    const [futureCareCosts, setFutureCareCosts] = useState("150000");
    const [householdSize, setHouseholdSize] = useState("4");
    const [result, setResult] = useState<ReturnType<typeof calculateWaterContaminationSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateWaterContaminationSettlement(contaminationType, healthEffect, exposureDuration, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(lostWages.replace(/\D/g, '')) || 0, parseInt(futureCareCosts.replace(/\D/g, '')) || 0, parseInt(householdSize) || 1));
    };
        ...
        <>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Water Contamination Calculator</h1>
                    <p className="text-slate-400">7 input fields for comprehensive settlement estimation.</p>
                </div>

                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Contamination Type</label><select value={contaminationType} onChange={(e) => setContaminationType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{CONTAMINATION_TYPES.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Health Effect</label><select value={healthEffect} onChange={(e) => setHealthEffect(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{HEALTH_EFFECTS.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Household Size</label><input type="number" value={householdSize} onChange={(e) => setHouseholdSize(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-amber-900/30 to-blue-900/20 border border-amber-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-amber-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.contaminationType}  {result.healthEffect}  {result.exposureDuration}</p></div>
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">Water contamination cases require evidence of exposure and diagnosis. Consult with an attorney experienced in PFAS or Camp Lejeune litigation.</p></div>
                    </div>
                )}
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="water-contamination" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{WATER_CONTAMINATION_2026.citations.join("  ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        mainEntity: [
                            {
                                "@type": "Question",
                                name: "How accurate is the water contamination calculator?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Is this water contamination calculator free to use?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "How often is the water contamination data updated?",
                                acceptedAnswer: {
                                    "@type": "Answer",
                            ...
                            {
                                "@type": "Question",
                                name: "Can I rely on these water contamination results for decisions?",
                                acceptedAnswer: {
                                    "@type": "Answer",
