    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 py-16 text-center">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm text-emerald-300">Know Your PMI Cost</span>
          </div>
          ...
          <div className="mt-8 bg-emerald-900/30 border border-emerald-700/50 rounded-lg p-4 max-w-xl mx-auto">
            <div className="flex items-center gap-2 text-emerald-300 text-sm">
              ...
              <span>PMI Removal: <strong>{PMI_2026.removalThresholds.automatic}% LTV</strong> (automatic)</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              <p className="text-3xl font-bold text-emerald-400">
                {formatCurrency(PMI_2026.statistics.avgPMI)}
              </p>
              <p className="text-sm text-slate-400 mt-1">Avg Monthly PMI</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-teal-400">
                {formatCurrency(PMI_2026.statistics.avgAnnualPMI)}
              </p>
              <p className="text-sm text-slate-400 mt-1">Avg Annual Cost</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-cyan-400">
                {PMI_2026.statistics.avgRemovalTime} yrs
              </p>
              <p className="text-sm text-slate-400 mt-1">Avg Time to Remove</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-green-400">
                {PMI_2026.statistics.homesWithPMI}%
              </p>
              <p className="text-sm text-slate-400 mt-1">Mortgages with PMI</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-800/30 border-y border-slate-700">
        <div className="max-w-4xl mx-auto px-4 py-16">
          <h2 className="text-2xl font-bold text-white mb-8 text-center">
            {SITE.year} PMI Rates by Down Payment
          </h2>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {PMI_2026.rates.map((tier) => (
              <div key={tier.description} className="bg-slate-800 rounded-xl p-4 border border-slate-700 text-center">
                <p className="text-emerald-400 font-bold text-2xl">{tier.rate}%</p>
                <p className="text-white font-medium text-sm mt-1">{100 - tier.ltvMax}% Down</p>
                <p className="text-xs text-slate-400 mt-1">{tier.ltvMax}% LTV</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-16 text-center">
        <h2 className="text-2xl font-bold text-white mb-4">
          Find Out Your PMI Cost
        </h2>
        <p className="text-slate-400 mb-8">
          See your monthly payment and get a removal timeline.
        </p>
        <Link
          href="/pmi/calculator"
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <p className="text-sm text-slate-400 text-center">
              {PMI_2026.citation}. For informational purposes only.
            </p>
