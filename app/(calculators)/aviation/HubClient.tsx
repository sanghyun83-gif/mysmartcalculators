...
export default function HubClient() {
  return (
    <div className="bg-[#020617] min-h-screen text-slate-300 font-sans selection:bg-indigo-500/30">
      {/* 1. S-Class Hero: Cinema-Indigo Aerospace Hub */}
        ...
        <div className="max-w-7xl mx-auto px-6 relative">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] animate-pulse">
              ...
              AEROSPACE PROTOCOL 2026: ACTIVE
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 pt-8">
              <Link href="/aviation/calculator" className="group relative px-12 py-6 bg-white text-slate-950 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-2xl overflow-hidden shadow-indigo-500/10">
              ...
              <div className="flex items-center gap-4 px-6 py-4 rounded-2xl bg-slate-900/50 border border-white/5 backdrop-blur-xl">
                <div className="text-left">
                  <p className="text-[10px] font-black text-white leading-none uppercase tracking-widest">FAA/NTSB DATA</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/80 backdrop-blur-2xl border border-white/5 p-8 rounded-[2.5rem] space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-widest">Aviation Profile</h3>
            </div>
            <table className="w-full text-left text-[11px] font-medium border-collapse">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="py-3 text-slate-500 uppercase tracking-tighter text-right">Stat</th>
                </tr>
              </thead>
              <tbody className="text-slate-300">
                <tr className="border-b border-white/5">
                  <td className="py-3 font-black text-white uppercase tracking-tighter">Annual Accidents</td>
                  <td className="py-3 text-right text-indigo-400">{AVIATION_2026.statistics.annualAccidents.toLocaleString()}</td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-3 uppercase font-black">Monthly Fatalities</td>
                  <td className="py-3 text-right text-[10px] text-red-400 font-bold">~29</td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-3 italic">Montreal Conv.</td>
                  <td className="py-3 text-right text-slate-400">{formatCurrency(AVIATION_2026.statistics.montrealConventionLimit)}</td>
                </tr>
              </tbody>
            </table>
          </div>

          {/* Table II: Accident Arbitrage */}
          <div className="bg-slate-900/80 backdrop-blur-2xl border border-white/5 p-8 rounded-[2.5rem] space-y-6">
            <div className="flex items-center gap-3">
              <Plane className="w-5 h-5 text-indigo-500" />
              <h3 className="text-sm font-black text-white uppercase tracking-widest">Crash Classes</h3>
            </div>
            <table className="w-full text-left text-[11px] font-medium border-collapse">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="py-3 text-slate-500 uppercase tracking-tighter">Aircraft</th>
                  <th className="py-3 text-slate-500 uppercase tracking-tighter text-right">Avg Settlement</th>
                </tr>
              </thead>
              <tbody className="text-slate-300">
                {AVIATION_2026.accidentTypes.slice(0, 4).map((acc, i) => (
                  <tr key={i} className="border-b border-white/5">
                    <td className="py-3 italic">{acc.type}</td>
                    <td className="py-3 text-right text-indigo-400">{formatCurrency(acc.avgSettlement)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-slate-900/80 backdrop-blur-2xl border border-white/5 p-8 rounded-[2.5rem] space-y-6">
            ...
            <table className="w-full text-left text-[11px] font-medium border-collapse">
              ...
              <tbody className="text-slate-300">
                <tr className="border-b border-white/5">
                  <td className="py-3 font-black text-white">Mfg Defect</td>
                  <td className="py-3 text-right text-indigo-400">1.6x Multiplier</td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-3 italic">Pilot Error</td>
                  <td className="py-3 text-right text-indigo-500 font-bold">Base Liability</td>
                </tr>
                <tr className="border-b border-white/5">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:flex gap-20 items-center">
          <div className="lg:w-1/2 space-y-8">
            ...
            <div className="grid gap-4">
              {[
                { title: "Black Box Forensics", desc: "Audit of NTSB factual reports and CVR logs", icon: Briefcase },
                { title: "Treaty Calibration", desc: "Montreal Convention 128,821 SDR sync", icon: Lock },
                { title: "Manufacturer Audit", desc: "Manufacturing defect liability roadmap", icon: Activity }
              ].map((item, i) => (
          ...
          <div className="lg:w-1/2 mt-16 lg:mt-0 relative">
            ...
            <div className="relative aspect-square bg-slate-900 border border-white/10 rounded-[4rem] p-12 overflow-hidden flex flex-col justify-between">
              <div className="absolute top-0 right-0 p-12 opacity-5">
                <Plane className="w-64 h-64 rotate-12 text-indigo-900" />
              </div>
              <div className="space-y-4">
                <div className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Liability Calibration</div>
      ...
      <section id="faq" className="py-32 bg-slate-950 border-y border-white/5 relative overflow-hidden">
        <div className="max-w-3xl mx-auto px-6 relative">
          ...
          <div className="grid gap-6">
            {[
              {
                q: "What is the average plane crash settlement in 2026?",
              ...
              {
                q: "Who is liable for a private aircraft crash?",
              ...
              {
                q: "How does NTSB data influence my claim?",
      ...
      <section className="py-32 bg-[#020617] relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="p-16 bg-gradient-to-br from-indigo-600 to-indigo-950 rounded-[4rem] text-center space-y-8 shadow-2xl relative overflow-hidden group shadow-indigo-500/20">
            ...
            <div className="relative space-y-6">
              <h2 className="text-5xl md:text-7xl font-black text-white tracking-tighter italic uppercase leading-none">
                Audit Your Aerospace <br />
                Recovery Potential.
              </h2>
              <p className="text-indigo-100 text-xl font-medium max-w-2xl mx-auto italic leading-relaxed">
                Don't let manufacturing defects or carrier negligence go unchallenged. Initiate your official S-Class forensic aviation liability audit.
              </p>
              <Link href="/aviation/calculator" className="inline-flex items-center gap-4 bg-white text-slate-950 px-14 py-7 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-50 transition-all hover:-translate-y-2 shadow-2xl">
          ...
          <div className="mt-32">
            <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
              {CALCULATORS.map((calc, i) => (
                <Link key={i} href={`/${calc.id}`} className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-indigo-500/50 transition-colors">
          ...
          <div className="mt-16">
            <RelatedCalculators currentCalc="aviation" count={6} />
          </div>
        </div>
      </section>
    </div>
