...
export default function KneeCalculatorPage() {
    ...
    const handleCalculate = () => { const medicalNum = parseVal(medicalBills); setResult(calculateKneeSettlement(compIndex, brandIndex, timeIndex, medicalNum)); };

    return (
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-4">
                    <Link href="/knee-implant" className="text-slate-400 hover:text-white"><ArrowLeft className="w-6 h-6" /></Link>
                    <div className="flex items-center gap-2"><span className="text-xl">🦵</span><span className="text-lg font-bold text-white">Knee Implant Calculator</span></div>
            ...
            <main className="max-w-2xl mx-auto px-4 py-8">
                ...
                <p className="mt-8 text-xs text-slate-500 text-center">{KNEE_2026.citation}</p>
            </main>
            <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [{ "@type": "Question", name: "Which knee implants have been recalled?", acceptedAnswer: { "@type": "Answer", text: "Exactech, Zimmer Persona, Stryker Triathlon, DePuy Attune, and Smith & Nephew Journey II." } }] }) }} />
        </div>
    );
}
