...
import { SITE, INJURY_TYPES, AMPUTATION_LEVEL, FAQS, calculateInvokanaSettlement, formatCurrency, INVOKANA_2026 } from "@/lib/calculators/invokana";
import { RelatedCalculators } from "@/components/RelatedCalculators";
import { Calculator, Heart, ChevronDown, ChevronUp, ArrowLeft, AlertTriangle } from "lucide-react";
import { LegalDisclaimer } from "@/components/LegalDisclaimer";

export default function CalculatorPage() {
    const [injuryType, setInjuryType] = useState("toe-amputation");
    const [amputationLevel, setAmputationLevel] = useState("toe");
    ...
    const [result, setResult] = useState<ReturnType<typeof calculateInvokanaSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateInvokanaSettlement(injuryType, amputationLevel, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, parseInt(monthsOnDrug) || 0, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            <header className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-50">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    <Link href="/invokana" className="flex items-center gap-2 hover:opacity-80"><Heart className="w-6 h-6 text-purple-500" /><span className="text-lg font-bold text-white">{SITE.name}</span></Link>
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Invokana Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Amputation Level</label><select value={amputationLevel} onChange={(e) => setAmputationLevel(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{AMPUTATION_LEVEL.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">Months on Invokana</label><input type="number" value={monthsOnDrug} onChange={(e) => setMonthsOnDrug(e.target.value)} min="0" className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white" /></div>
                    </div>
                    <label className="flex items-center gap-3 p-3 bg-slate-700/50 rounded-lg cursor-pointer mt-4"><input type="checkbox" checked={hasDocumentation} onChange={() => setHasDocumentation(!hasDocumentation)} className="w-4 h-4 text-purple-500" /><span className="text-slate-300 text-sm">Have pharmacy/prescription records for Invokana</span></label>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-rose-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-6"><p className="text-4xl md:text-5xl font-bold text-purple-400">{formatCurrency(result.totalLow)} - {formatCurrency(result.totalHigh)}</p><p className="text-slate-400 mt-2">{result.injuryType} • {result.amputationLevel}</p></div>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8"><div className="flex justify-center"><div className="w-full max-w-xs"><RelatedCalculators currentCalc="invokana" count={5} /></div></div>
      <section className="max-w-4xl mx-auto px-4 py-4"><LegalDisclaimer category="legal" /></section>
</section>

            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{INVOKANA_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
