...
import { SITE, INJURY_TYPES, ATRIUM_PRODUCTS, FAQS, calculateAtriumSettlement, formatCurrency, ATRIUM_2026 } from "@/lib/calculators/atrium-mesh";
...
export default function CalculatorPage() {
    ...
    const [productType, setProductType] = useState("c-qur-v-patch");
    ...
    const [result, setResult] = useState<ReturnType<typeof calculateAtriumSettlement> | null>(null);
    const [openFaq, setOpenFaq] = useState<number | null>(null);

    const handleCalculate = () => {
        setResult(calculateAtriumSettlement(injuryType, productType, parseInt(medicalExpenses.replace(/\D/g, '')) || 0, hadRevisionSurgery, hasDocumentation));
    };
        ...
        <div className="min-h-screen bg-slate-900">
            ...
            <section className="max-w-4xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">Atrium C-Qur Settlement Calculator</h1>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-300 mb-2">C-Qur Product</label><select value={productType} onChange={(e) => setProductType(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white">{ATRIUM_PRODUCTS.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                ...
                {result && (
                    <div className="mt-8 bg-gradient-to-br from-purple-900/30 to-red-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8">
                        ...
                        <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-sm text-amber-200">C-Qur cases with documented omega-3 coating reactions receive highest compensation in MDL 2753.</p></div>
                    </div>
                )}
            </section>
            ...
            <footer className="bg-slate-800 border-t border-slate-700"><div className="max-w-6xl mx-auto px-4 py-8 text-center"><p className="text-sm text-slate-400">{ATRIUM_2026.citations.join(" • ")}</p><p className="text-sm text-slate-500 mt-2">© {SITE.year}</p></div></footer>
        </div>
    );
}
